...
对 AISE 官网首页进行视觉改版并配套建设运营管理后台，使首页内容（导航、轮播图、新闻、专家、合作单位）完全由运营人员自主配置，无需研发介入。
...
- **用户收益**：
  - 首页内容更新周期从"提需求→排期→研发→发布"的 3-5 天缩短至运营人员后台即时生效（减少 95% 以上）
  - 新闻中心支持分类筛选与无限滚动，信息查找效率提升 50%
- **业务收益**：
  - 运营人员可自助管理全部首页模块，预计减少 90% 的研发日常配置支持工作量（基于过去 3 个月平均每月配置需求 15+ 工单）
...
| 版本号 | 日期 | 修订人 | 修订内容 |
|--------|------|--------|---------|
| V1.0 | 2026-05-12 | 2red-prd-skill | 初稿 |
...
| 涉及终端 | Web（PC 端），适配 1280px-1920px 主流分辨率 |
| 目标上线版本 | [目标版本号] |
...
**5.3.2 交互逻辑**
- 卡片悬停：整体上浮 4px，底部投影扩大并带品牌色光晕，边框转浅蓝；内部图标放大 110%，主标题变品牌色，过渡 300ms
- 点击卡片：跳转至对应业务页面

**5.3.3 加载态**
- 首屏加载时显示 3 个矩形卡片占位骨架（浅灰色 + 圆形图标占位）
...
**5.7.3 交互逻辑**
- 点击"新增一级栏目"：在列表顶部新增空行
- 保存：校验通过后保存，Toast 提示"保存成功"（3s），首页即时生效
- 保存失败：红色轻提示"保存失败：{失败原因}"，保留已填数据

**5.7.4 加载态**
- 页面首次加载时列表区域显示 5 行浅灰色占位条
- 保存操作中按钮变为"保存中..."并置灰禁用

**5.7.5 错误态**
- 列表接口加载失败：显示"加载失败" + "重试"按钮
- 保存接口失败：同 5.7.3 保存失败处理
...
**5.9.3 加载态**
- 列表首次加载时显示 4 行占位条（含圆形头像占位 + 文字占位）

**5.9.4 错误态**
- 头像未裁剪为 1:1 时，保存按钮旁显示红色提示"请裁剪为 1:1 比例"
- 列表接口失败：显示"加载失败" + "重试"按钮
...
- 未勾选：仅在独立列表页可见，可将 URL 配置到其他入口
- 创建后自动生成分类页路径（如 `/news/category/{slug}`），提供一键复制及跳转预览
...
| 3 | 导航栏菜单收起 | 二级菜单已展开 | 鼠标移出一级栏目区域 | 延时 200ms 后二级菜单收起 | 延时准确 |
| 4 | 导航接口失败降级 | 模拟导航接口超时 | 刷新首页 | 显示"导航加载异常"及重试按钮，默认展示首页+关于 | 降级显示正确 |
| 5 | 轮播图自动播放 | 已配置 ≥ 2 张轮播图 | 等待 5 秒 | 轮播图自动切换到下一张 | 5 秒后自动切换 |
| 6 | 轮播图手动切换重置计时 | 轮播图自动播放中 | 点击左右切换按钮 | 切换到对应图片，自动播放计时重置 | 切换正确，计时重置 |
| 7 | 轮播图加载失败占位 | 图片 URL 无效 | 刷新首页 | 显示占位图，左上角轻提示"图片加载失败" | 占位图正确展示 |
| 8 | 业务入口悬停效果 | 首页正常加载 | 鼠标悬停入口卡片 | 卡片上浮 4px，阴影带品牌色光晕，图标放大 110% | 动效正确 |
| 9 | 新闻中心无限滚动 | 新闻总量 > 20 条 | 滚动至列表底部 | 自动加载下一页 20 条，显示"加载中..." | 新数据追加显示 |
| 10 | 新闻中心分类切换 | 已配置 ≥ 2 个首页展示分类 | 点击不同 Tab | 列表刷新为对应分类内容 | 内容正确切换 |
| 11 | 新闻中心无数据 | 后台无任何新闻 | 访问首页新闻区域 | 空状态插画 + "暂无新闻资讯" | 空状态正确 |
| 12 | 专家团队骨架加载 | 已配置 ≥ 4 位专家 | 刷新首页 | 加载中显示 4 个圆形占位头像 | 骨架屏正确展示 |
| 13 | 合作单位按分类分组 | 已配置 3 个分类各 ≥ 2 个单位 | 访问首页合作单位区域 | 3 个分类以分割线隔开，各单位在对应分组下 | 分组正确 |
| 14 | 合作单位链接跳转 | 某合作单位配置了跳转链接 | 点击该单位名称 | 新标签页打开目标链接 | 跳转正确 |
| 15 | 顶栏配置新增栏目 | 运营后台顶栏管理页 | 新增一级栏目，填写名称和序号，保存 | Toast"保存成功"，首页即时显示 | 保存并即时生效 |
| 16 | 顶栏配置序号冲突 | 已有栏目序号为 1 | 新增栏目序号也为 1，保存 | 系统自动将新增及后续序号 +1 | 序号唯一且连续 |
| 17 | 顶栏配置删除父级栏目 | 存在带二级栏目的一级栏目 | 点击删除，确认弹窗 | 父级及子级同步删除 | 级联删除正确 |
...
| 21 | 文章分类创建并自动生成 URL | 文章分类管理页 | 输入分类名称，保存 | Toast"创建成功"，展示 URL 及复制按钮 | URL 生成正确 |
| 22 | 文章分类删除不影响文章 | 已有文章归属于分类 A | 删除分类 A | 文章未删除，文章归属分类字段置空 | 文章保留 |
| 23 | 首屏性能 | 首页正常网络环境 | 刷新首页 | 首屏加载完成时间 ≤ 2s | 首屏耗时达标 |
| 24 | 后台保存响应性能 | 运营后台任一模块 | 填写合法数据，点击保存 | Toast 提示在 500ms 内出现 | 保存响应速度达标 |
...
| 材料类型 | 涉及功能 | 状态 |
|---------|---------|------|
| UI 设计稿 | 用户端全部 6 个首页模块 | 需补充 |
| 运营后台页面原型 | 运营后台全部 5 个管理模块 | 需补充 |
| 流程图 | 首页加载流程、运营后台配置发布流程 | 已提供 Mermaid |
| 设计规范 | 品牌视觉定位（配色方案、字体规范） | 建议补充独立设计规范文档链接 |
