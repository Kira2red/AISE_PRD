...
为 AISE 测评报名平台接入微信支付，替代现有「静态二维码 + 用户手动点击确认」的线下转账模式，实现从支付到报名成功的全流程自动闭环。同时借支付环节引导用户关注公众号，建立后续触达能力。

### 1.2 业务诉求

- 报名支付效率低：当前用户扫码支付后须回到页面手动点击「我已完成支付」，流程断裂、体验差
- 缺乏自动化支付验证：系统无法感知支付是否完成，依赖用户自我报告
- 需要服务号触达通道：支付成功后推送测评安排通知，需要先完成用户关注和账号绑定
