...
**可展示题型清单**：报告仅展示以下题型的作答——单选题、多选题、判断题、填空题（客观）；OJ 编程题、编程题（主观，作答为代码文本）。其余题型（图形化、实践题、AIGC 文生文、AIGC 文生图、AI 工作流、划线题等）不进入本模块选题范围。所有题型均参与知识点掌握率计算（见 5.1.4），与展示范围无关。

按总分是否满分分两套展示逻辑：

**满分画像（总分 = 100）**：

- 展示 1 道题——首道满分的 OJ 编程题或编程题。
- 只展示学生作答，不展示正确答案。
- 点评为固定文案「本题目作答达到要求，完整完成任务。太棒了！」。

**非满分画像（总分 < 100）**：

- 展示首道错误的客观题 + 首道扣分的 OJ 编程题或编程题。
- 客观题展示题干、学生选项与正确选项（含「你的作答」「正确答案」标注）；代码题展示学生作答代码。
- 两道题按题号排序，客观题在前。
- 点评为「建议重点提升【题目二级标签知识点】。」。

**缺一展一原则**：

- 客观题与代码题各自独立判定：客观题有错则展示客观题，代码题有扣分则展示代码题，两者缺一就只展示存在的部分。
- 客观题全对时只展示代码题；代码题无扣分时只展示客观题。
- 两者均无（非满分但无错题可展示）时，该模块整体不展示。
- 满分场景按规则找不到首道满分代码题时，该模块同样不展示，不留空卡片。
- 题目作答题型不在可展示清单内（如作品提交类题型）时，该题视为不存在、不参与选题；由此导致无可展示题目时，模块整体不展示。

**题目呈现**：

- 客观题以题干 + 学生选项 + 正确选项的形式展示。
- 代码题以深色代码块展示学生作答代码；作答代码行数较多时统一保留前 5 行后截断，截断处显示省略提示。
...
| 12 | 题型过滤 | 考生错题/扣分题为图形化等作品提交类题型（不在可展示清单内） | 查看报告 | 该题不展示、不参与选题；若因此无可展示题，该模块整体不展示 | 不出现作品题作答 |
