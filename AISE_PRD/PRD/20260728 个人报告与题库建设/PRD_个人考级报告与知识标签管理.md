...
| V1.3 | 2026-09-10 | 报告生成时机由成绩发布提前至批改完成（数据分层：作答数据批改完成落库，等级维持实时计算）；新增 5.3.1 运营端报告预览（admin/14 加「查看报告」入口、admin/16 电子证书按钮改造）；出处：微信群 2026-09-10 需求讨论，张二红拍板 | 产品 |
...
| 测评系统 | 研发 | 提供正式总分、题目知识点、学生作答、题目满分值 |
...
考生在测评批改完成、成绩正式发布后，系统自动为考生生成个人报告。考生从「我的活动」或「成绩与证书」查询结果进入报告页，查看等级结论、成长区间、知识点掌握情况、错题复盘与下一阶段建议，并可下载 PDF 版本保存。
...
```
阶段:  批改完成 ────────→ 报告数据生成 ─────→ 成绩公示 ────────→ 用户查看/下载
        │                  │                  │                  │
报告生成 │  批改完成事件触发  异步任务生成        公示后学生端可见    服务端渲染
（后台） │  （独立入队）      作答数据落库        入口按钮展示        PDF 供保存
        │                  等级实时计算                              │
        │  ┌─────────────┐  │                                        │
        │  │ 运营预览     │ ←┘ 批改完成后、公示前，admin/14→16 预览    │
        │  └─────────────┘                                           │
────────┼──────────────────┼──────────────────┼──────────────────┼────────
我的活动 │                                        ┌─────────────┐  │
（user/15）                                   │ 测评卡片新增  │  │
...
| 数据                | 来源                           | 用途                 |
| ----------------- | ---------------------------- | ------------------ |
| 正式总分              | 测评系统发布成绩                     | 等级映射、通过判定、满分/非满分判定 |
| 题目知识点（一级/二级/三级标签） | 题目录入（独立 PRD 建设），知识树清单由运营人工补充 | 掌握度统计、错题点评的知识点引用   |
| 题目作答（选项/代码）       | 测评系统                         | 考级具体表现展示           |
| 题目满分值             | 测评系统                         | 掌握率计算              |
| 级别、姓名、测评年月        | 报名/测评数据                      | 标题栏、建议文案           |
...
**目标**：向考生展示一场考级测评的完整学习反馈，涵盖等级结论、知识点掌握、错题复盘与学习建议。
...
- 操作区为正式版固定结构，包含两部分：当前考生信息摘要（姓名、级别、测评年月、总分与报告等级）与「下载 PDF 报告」按钮。操作区无其他操作项；Demo 原型中的考生切换为演示工具（用于演示满分/非满分/未通过/知识点未配置四种报告形态），正式版不实现。
- 页面同时适配 PC 与手机浏览器，移动端适配口径见验收标准。
- 页面支持两种进入方式，从进入方式带出考生信息：
  - 登录态：从「我的活动」进入，直接展示当前登录考生对应测评的报告。
  - 查询态：从「成绩与证书」查询结果进入，展示查询命中的考生报告，报告通过测评记录标识定位，标识不可被转发盗用；查询态姓名完整展示，不脱敏。
...
- 报告主标题按「测评年月 + 级别 + 个人报告」格式组织（如「2026年8月程序设计四级个人报告」），测评年月取本次测评实际月份，级别如「程序设计四级」；眉题展示报告品牌（如「AISE能力评定报告」）。
- 标题栏信息展示考生姓名、通过/未通过状态；级别与测评年月由主标题承载，不重复单列。
...
- 直接使用测评系统正式发布总分，不取整、不四舍五入、不重新汇总。
- 等级映射表如下（原始需求原文照录）：
...
- 考生没有任何题目关联知识点时（如整场测评题目均未配置知识点），本模块显示「暂无知识点掌握数据」占位说明，其余模块正常展示。
- 知识树中存在但本场测评无任何关联题目的二级知识点不展示（掌握率分母为 0，不参与排序与展示）。

#### 5.1.5 考级具体表现

**可展示题型清单**：报告仅展示以下题型的作答——单选题、多选题、判断题、填空题（客观）；OJ 编程题（主观，作答为文本代码）。其余题型（编程题、图形化编程、实践题、AIGC 文生文、AIGC 文生图、AI 工作流、划线题等，均为提交文件作答）不进入本模块选题范围。所有题型均参与知识点掌握率计算（见 5.1.4），与展示范围无关。

按总分是否满分分两套展示逻辑：

**满分画像（总分 = 100）**：

- 展示 1 道题——首道满分的 OJ 编程题。
- 只展示学生作答，不展示正确答案。
- 点评为固定文案「本题目作答达到要求，完整完成任务。太棒了！」。

**非满分画像（总分 < 100）**：

- 展示首道错误的客观题 + 首道扣分的 OJ 编程题。
- 客观题展示题干、学生选项与正确选项（含「你的作答」「正确答案」标注）；OJ 编程题展示学生作答代码。
- 两道题按题号排序，客观题在前。
- 点评为「建议重点提升【题目二级标签知识点】。」。

**缺一展一原则**：

- 客观题与 OJ 编程题各自独立判定：客观题有错则展示客观题，OJ 编程题有扣分则展示 OJ 编程题，两者缺一就只展示存在的部分。
- 客观题全对时只展示 OJ 编程题；OJ 编程题无扣分时只展示客观题。
- 两者均无（非满分但无错题可展示）时，该模块整体不展示。
- 满分场景按规则找不到首道满分 OJ 编程题时，该模块同样不展示，不留空卡片。
- 题目作答题型不在可展示清单内（如提交文件作答的编程题、图形化编程等）时，该题视为不存在、不参与选题；由此导致无可展示题目时，模块整体不展示。

**题目呈现**：

- 客观题以题干 + 学生选项 + 正确选项的形式展示。
- OJ 编程题以深色代码块展示学生作答代码；作答代码行数较多时统一保留前 5 行后截断，截断处显示省略提示。
...
- 现状：测评卡片操作区有「查看准考证」「模拟测评」「正式测评/进入测评/你已交卷」等按钮。
- 改造：成绩已发布且报告已生成完成的卡片，在操作区新增「查看报告」按钮；成绩未发布或报告生成中的卡片不展示该按钮。
- 按钮样式与同操作区其他按钮一致，点击后在新页面打开该测评对应考生的报告。

#### 5.2.2 「成绩与证书」查询页（user/19、user/20，改造）

**功能描述**：

- 现状：输入姓名、手机号（可选证书编号）查询测评列表；结果卡片有 5 种状态：已签发（查看电子证书）、待签发、证书生成中、未获得证书（成绩未达标）、等待成绩。其中「未获得证书」的考生恰恰有报告无证书，是最需要「查看报告」入口的场景。
...
- **触发时机（2026-09-10 改向）**：批改完成（批改进入待复核）事件触发，即报告作答数据在成绩公示前生成。报告任务独立入队，不与证书申请接口耦合——证书由申请触发且仅覆盖成绩达标考生；报告覆盖所有已批改且有正式总分的考生（含未通过）。若挂在证书申请上，未达标考生（不申请证书）将永远没有报告，与覆盖范围要求矛盾。
- **数据分层（与 20260909 报告等级实时计算口径兼容）**：批改完成时生成的是**作答数据部分**（知识点掌握度、考级具体表现、下一阶段建议——仅依赖批改结果）；等级名、通过状态、成长区间、一句话总结维持 20260909 口径，按查看时点的当前报告规则实时计算，不存储。公示前修改报告规则，预览与学生端公示后所见同步反映新规则。
...
**待确认事项**：~~具体触发挂接方式（成绩发布事件监听点）与证书侧对齐后确定。~~ → 2026-09-10 改为批改完成事件（见上方触发时机）。

#### 5.3.1 运营端报告预览（admin/14、admin/16，2026-09-10 新增）

**背景**：运营需在成绩公示前校验报告内容是否准确；同时 16 页原「电子证书」按钮（点击提示「暂无电子证书」）功能重复，改造为报告预览入口。

**入口改造**：

| 页面 | 改造 |
|------|------|
| 活动证书管理（admin/14） | 操作列新增「查看报告」按钮，点击进入学生证书管理页（admin/16）并自动带上该测评的筛选条件；按钮在该测评批改进度未完成时置灰 |
| 学生证书管理（admin/16） | 原电子证书下载按钮（未签发行点击提示「暂无电子证书」）改名「查看报告」，点击新页面打开该学生的报告详情页；已签发行保留「下载电子证书」能力不变 |

**预览规则**：

- 预览页复用用户端报告详情页（user/29）同一渲染，预览样式与学生所见电子版样式完全一致。
- 预览可见时间：批改完成后即可（成绩公示前）；学生端不变，仍成绩公示后可见。
- 预览内容按查看时点实时计算（与 5.3 数据分层一致）；页面顶部提示「报告按当前规则实时计算，学生端于成绩公示后可见」。
- 复核驳回重新批改时，报告作答数据随重新批改结果覆盖刷新。
...
| 12 | 题型过滤 | 考生错题/扣分题为编程题、图形化编程等提交文件作答题型（不在可展示清单内） | 查看报告 | 该题不展示、不参与选题；若因此无可展示题，该模块整体不展示 | 不出现文件类作答 |
| 13 | 建议文案 | 各级别通过/未通过 | 查看报告 | 考级衔接与课程学习建议逐字照录原始需求 1.6；未通过不展示课程学习建议 | 文案逐字一致 |
| 14 | 三级文案缺口 | 三级考生报告 | 查看报告 | 建议模块不展示（文案未配置），其余模块正常 | 无空白卡片 |
| 15 | 知识点空态 | 整场测评题目未配置知识点 | 查看报告 | 该模块显示「暂无知识点掌握数据」占位，其余模块正常 | 占位正确 |
| 16 | 加载态 | 报告数据未就绪 | 打开报告页 | 展示加载状态，无空白页 | 加载态可见 |
| 17 | 错误态 | 报告生成失败 | 打开报告页 | 展示友好错误提示与「重新加载」操作 | 提示与重试可用 |
| 18 | 操作区结构 | 报告已生成 | 打开报告页 | 操作区仅含考生信息摘要（姓名/级别/测评年月/总分/等级）与「下载 PDF 报告」按钮，无考生切换等其他操作 | 结构与正式版定义一致 |
| 19 | 动态布局-窄屏 | 移动端/窄屏 | 打开报告页 | 操作区在上、报告在下，纵向滚动 | 布局正确 |
| 20 | 字号验收 | 移动端 390 宽度 | 打开报告页 | 正文和题目内容不低于 15px、辅助标签不低于 12px、核心标题 20-24px | 字号达标 |
| 21 | PDF 下载 | 报告已生成 | 点击「下载 PDF 报告」 | 生成期间按钮置灰提示生成中，成功后下载 PDF，失败提示可重试 | 全流程正确 |
| 22 | PDF 样式 | 打开下载的 PDF | 下载并打开 | PDF 样式与线上报告页一致（标题栏/概览/知识点/表现/建议完整） | 样式一致 |
...
| # | 测试场景 | 前置条件 | 操作 | 期望结果 | 判定 |
|---|---------|---------|------|---------|------|
| 27 | 触发与覆盖 | 测评批改完成（进入待复核） | 触发批改完成事件 | 所有已批改且有正式总分的考生（含未通过）均生成作答数据，与证书申请无关；等级内容按查看时点实时计算 | 覆盖范围与数据分层正确 |
| 28 | 失败重试 | 生成任务失败 | 观察任务状态 | 自动重试；重试仍失败记录日志，入口不展示；重试成功后恢复 | 重试与恢复正确 |
| 28a | 运营预览入口 | 测评批改完成 | admin/14 点「查看报告」→ admin/16 点该学生「查看报告」 | 新页面打开报告详情，样式与学生端一致；页面顶部有实时计算与公示后可见提示 | 预览可用 |
| 28b | 预览时限 | 测评已批改完成、成绩未公示 | 运营预览任一学生报告 | 运营可见；学生端（我的活动/成绩与证书）不展示「查看报告」 | 运营可见学生不可见 |
| 28c | 批改未完成置灰 | 测评批改进度未 100% | 查看 admin/14 操作列 | 「查看报告」按钮置灰不可点 | 置灰正确 |
| 28d | 规则变更同步 | 批改完成、成绩未公示，运营预览中 | 管理员修改该场报告等级规则后刷新预览 | 预览立即反映新规则；学生公示后所见与预览一致 | 实时计算生效 |
| 28e | 复核驳回刷新 | 学生报告已生成，该生复核驳回重新批改 | 重新批改完成后运营再预览 | 作答数据按重新批改结果刷新 | 数据覆盖正确 |
