...
- 报告主标题按「考试年月 + 级别 + 个人报告」格式组织（如「2026年8月程序设计四级个人报告」），考试年月取本次考试实际月份，级别如「程序设计四级」；眉题展示报告品牌（如「AISE能力评定报告」）。
- 标题栏信息展示考生姓名、通过/未通过状态；级别与考试年月由主标题承载，不重复单列。
...
| 15 | 知识点空态 | 整场考试题目未配置知识点 | 查看报告 | 该模块显示「暂无知识点掌握数据」占位，其余模块正常 | 占位正确 |
| 16 | 加载态 | 报告数据未就绪 | 打开报告页 | 展示加载状态，无空白页 | 加载态可见 |
| 17 | 错误态 | 报告生成失败 | 打开报告页 | 展示友好错误提示与「重新加载」操作 | 提示与重试可用 |
| 18 | 操作区结构 | 报告已生成 | 打开报告页 | 操作区仅含考生信息摘要（姓名/级别/考试年月/总分/等级）与「下载 PDF 报告」按钮，无考生切换等其他操作 | 结构与正式版定义一致 |
...
| # | 测试场景 | 前置条件 | 操作 | 期望结果 | 判定 |
|---|---------|---------|------|---------|------|
| 27 | 触发与覆盖 | 考试批改完成（进入待复核） | 触发批改完成事件 | 所有已批改且有正式总分的考生（含未通过）均生成作答数据，与证书申请无关；等级内容按查看时点实时计算 | 覆盖范围与数据分层正确 |
